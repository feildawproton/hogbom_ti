...
class FitsFile(object):
    ...
    def __fill_reduced_grid(self):                
        for x_ndx, y_ndx in self.half_grid:
            for f_ndx in range(0, self.n_good):
                ...
                if cnu < HWIDTH and cnv < HWIDTH:
                    ...
                    self.half_grid[x_ndx, y_ndx][0] +=       wgt*self.real[f_ndx]
                    self.half_grid[x_ndx, y_ndx][1] += hflag*wgt*self.imag[f_ndx]
                    self.half_cnts[x_ndx, y_ndx]    += 1
                    self.half_bm[x_ndx, y_ndx][0]   += wgt
                ...
                if (x_ndx) < HWIDTH and (mu/self.du) < HWIDTH:
                    ...
                    if cnu < HWIDTH and cnv < HWIDTH:
                        wgt = (4.6*cnu+NCGF-0.5)*(4.6*cnv+NCGF-0.5)
                        self.half_grid[x_ndx, y_ndx][0] +=           wgt*self.real[f_ndx]
                        self.half_grid[x_ndx, y_ndx][1] += -1.*hflag*wgt*self.imag[f_ndx]
                        self.half_cnts[x_ndx, y_ndx]    += 1
                        self.half_bm[x_ndx, y_ndx][0]   += wgt
    ...
    def convert_to_grid(self, grid_dim_len: int, pix_arcsecs: float, briggs_weight: float) -> np.ndarray:
        ...
        if self.is_image == True:
            ...
            u_max      = int(np.ceil(np.max(np.fabs(u))))
            v_max      = int(np.ceil(np.max(np.fabs(v))))
            ...
            if self.use_ti == True:
                ...
                self.half_grid = ti.Vector.field(n=2, shape=(x_red_size, y_red_size), dtype=ti.f32)
                self.half_cnts = ti.field(shape=(x_red_size, y_red_size), dtype=ti.int32)
                self.half_bm   = ti.Vector.field(n=2, shape=(x_red_size, y_red_size), dtype=ti.f32)
                
                # 1.) GRID
                # 1.1) fill in the reduced grids
                self.__fill_reduced_grid()
...
if __name__ == "__main__":
    ...
    fits_file = FitsFile(path="demo_data/sim1.mickey.alma.out20.ms.fits", use_ti=False)
    grid_dim_len = 32 # characteristic size of the image, size will be grid_dim_len*grid_dim_len
    pix_arcsecs  = (grid_dim_len/100.) / grid_dim_len
    briggs_weight = 1e3
